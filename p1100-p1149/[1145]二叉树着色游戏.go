...
func btreeGameWinningMove(root *TreeNode, n int, x int) bool {
	...
	var dfs func(node *TreeNode, inLeft, inRight bool)
	dfs = func(node *TreeNode, inLeft, inRight bool) {
		if node == nil {
			return
		}
		if inLeft {
		...
		if node.Val == x {
			...
			dfs(node.Left, inLeft, inRight)
			dfs(node.Right, inLeft, inRight)
		}
	}
	dfs(root, false, false)
	min := n / 2
	return leftCnt > min || rightCnt > min || (leftCnt+rightCnt) < min
}
