...
func braceExpansionII(expression string) []string {
	...
	ope := func() {
		...
		if op[len(op)-1] == '+' {
			data[l] = append(data[l], data[r]...)
		} else {
			temp := []string{}
			for _, left := range data[l] {
				for _, right := range data[r] {
	...
	for i, ch := range expression {
		...
		case '{':
			...
			if i > 0 && (expression[i-1] == '}' || unicode.IsLetter(rune(expression[i-1]))) {
				op = append(op, '*')
			}
			op = append(op, '{')
		case '}':
		...
		default:
			...
			data = append(data, []string{string(expression[i])})
		}
	}
	for len(op) > 0 {
		ope()
	...
	result := []string{}
	for _, str := range data[len(data)-1] {
		if !checkMap[str] {
