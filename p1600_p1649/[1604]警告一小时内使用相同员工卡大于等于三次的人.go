...
//
//
// Related Topics 数组 哈希表 字符串 排序 👍 48 👎 0

//leetcode submit region begin(Prohibit modification and deletion)
func alertNames(keyName []string, keyTime []string) []string {
	n := len(keyName)
	timeFunc := func(timeStr string) int {
		hour, _ := strconv.Atoi(timeStr[0:2])
		min, _ := strconv.Atoi(timeStr[3:5])
		return hour*60 + min
	}
	m := make(map[string][]int, len(keyName))
	for i := 0; i < n; i++ {
		m[keyName[i]] = append(m[keyName[i]], timeFunc(keyTime[i]))
	}
